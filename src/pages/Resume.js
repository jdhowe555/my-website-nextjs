...
let jobRoles = [
    {
        ...
        technology: ['Splunk', 'Microsoft Graph', 'Microsoft Identity', '.NET 6', 'Oracle', 'IronSoftware', 'TinyMCE']
    },
    {
        name: 'UST',
        ...
        <div className="flex w-full min-w-screen justify-center min-h-[calc(100vh_-_4rem)]">
            <div className="flex flex-col md:flex-row gap-4 py-24">
                ...
                <div className="flex flex-col gap-10 w-[40rem]">
                    <section id="about">
                        <p className="p-4">I want to put something here, for the about me section of this but i dont know what to put here yet and im just going to ramble on so that this text is just long enough to test stuff with</p>
                    </section>
                    <section id="job-roles" className="flex flex-col gap-10">
                    {jobRoles.map((role) => (
                        <div key={role.name} className="rounded hover:bg-slate-700/50 transition motion-reduce:transition-none">
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    <ul className="flex flex-row gap-x-1.5 gap-y-2">
                                        {role.languages.map((language, i) => (
                                            <li key={i}>
                                                <div className="flex items-center rounded-full bg-teal-400/10 text-teal-300 px-3 py-1 text-xs font-medium leading-5">{language}</div>
                                            </li>
                                        ))}
                                    </ul>
                                    <ul className="flex flex-row flex-wrap gap-x-1.5 gap-y-2">
                                        {role.technology.map((language, i) => (
                                            <li key={i}>
                                                <div className="flex items-center rounded-full bg-sky-400/10 text-sky-300 px-3 py-1 text-xs font-medium leading-5">{language}</div>
                                            </li>
                                        ))}
                                    </ul>
                                </div>
